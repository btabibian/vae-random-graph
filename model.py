...
class VAEG(VAEGConfig):
    def __init__(self, hparams, placeholders, num_nodes, num_features, edges, istest=False):
        ...
        def masked_gen(posscore, negscore):
            ...
            for (u, v) in self.edges[self.count]:
                ...
                ll += tf.log(tf.multiply(tf.reshape(posscore[u], [1, self.n]), indicator[u])[
                             0][v] / (temp_posscore + negscore[u][v]) + 1e-09)
                ll += tf.log(tf.multiply(tf.reshape(posscore[v], [1, self.n]), indicator[v])[
                             0][u] / (temp_posscore + negscore[v][u]) + 1e-09)

                indicator[u] = np.multiply(tf.subtract(
                    tf.ones([1, self.n]), self.adj[v]), indicator[u])
                indicator[v] = np.multiply(tf.subtract(
                    tf.ones([1, self.n]), self.adj[u]), indicator[v])
        ...
        def neg_loglikelihood(prob_dict):
            ...
            with tf.variable_scope('NLL'):
                ...
                ll = tf.reduce_sum(
                    tf.log(tf.add(tf.multiply(self.adj, softmax_out), tf.fill([self.n, self.n], 1e-9))), 1)
        ...
        def kl_gaussian(mu_1, sigma_1, debug_sigma, mu_2, sigma_2):
            ...
            with tf.variable_scope("kl_gaussisan"):
                ...
                print("debug KL", first_term.shape, second_term.shape,
                      k.shape, third_term.shape, sigma_1[0].shape)
        ...
        self.input_data = tf.placeholder(dtype=tf.float32, shape=[
                                         self.k, self.n, self.d], name='input')
        ...
        self.grad_placeholder = [
            (tf.placeholder("float", shape=gr[1].get_shape()), gr[1]) for gr in self.grad]
    ...
    def plot_hspace(self, hparams, placeholders, num):
        ...
        for i in range(len(adj)):
            ...
            feed_dict = construct_feed_dict(
                hparams.learning_rate, hparams.dropout_rate, self.k, self.n, self.d, hparams.decay_rate, placeholders)
            feed_dict.update({self.adj: adj[i]})
    ...
    def kl_gaussian_mul(self, mu_1, sigma_1,  mu_2, sigma_2):
        ...
        for i in range(self.n):
            term_3.append(np.matmul(np.matmul(np.transpose(
                diff_mean[i]), inverse_sigma_2[i]), diff_mean[i]))
        ...
        for i in range(len(adj)):
            ...
            for j in range(10):
                ...
                prob, ll, z_encoded, enc_mu, enc_sigma, loss, kl = self.sess.run(
                    [self.prob, self.ll, self.z_encoded, self.enc_mu, self.enc_sigma, self.cost, self.kl], feed_dict=feed_dict)
                ll_total += np.mean(ll)
    ...
    def getcandidate(self, num, n, p, prob, list_edges):
        ...
        for k in range(num - 1):
            ...
            #unseen[u] = 0
            #unseen[v] = 0
            indicator[u] = np.multiply(np.multiply(
                np.subtract(np.ones(n), adj[v]), indicator[u]), unseen)
            indicator[v] = np.multiply(np.multiply(
                np.subtract(np.ones(n), adj[u]), indicator[v]), unseen)
    ...
    def sample_graph(self, hparams, placeholders, s_num, node, num=10, outdir=None, eps_passed=None):
        ...
        for i in range(len(adj)):
            ...
            if hparams.mask_weight:
                ...
                candidate_edges = [list_edges[i]
                                   for i in np.random.choice(range(len(list_edges)), [num], p=p)]
        ...
        if hparams.mask_weight:
            ...
            candidate_edges = [list_edges[i] for i in np.random.choice(
