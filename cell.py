...
class VAEGCell(object):
    ...
    def __call__(self, c_x, n, d, k, eps_passed, sample, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            c_x = input_layer(c_x, self.adj, self.features, k, n, d,
                              activation=None, batch_norm=False, istrain=False, scope=None)
            ...
            with tf.variable_scope("Encoder"):
                ...
                enc_hidden = fc_layer(tf.concat(list_cx, 1), k * d,
                                      activation=tf.nn.relu, scope="hidden")
                ...
                debug_sigma = fc_layer(enc_hidden, self.z_dim,
                                       activation=tf.nn.softplus, scope='sigma')
            ...
            with tf.variable_scope("Decoder"):
                ...
                for u in range(n):
                    for v in range(n):
                        ...
                        z_stack.append(
                            tf.concat(values=(tf.transpose(z[u]), tf.transpose(z[v])), axis=1)[0])
                dec_hidden = fc_layer(tf.stack(z_stack), 1,
                                      activation=tf.nn.softplus, scope="hidden")
