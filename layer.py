...
def input_layer(c_mat, adj, feature, k, n, d, activation=None, batch_norm=False, istrain=False, scope=None):
    ...
    for i in range(k):
        if i > 0:
            output_list.append(tf.multiply(
                tf.matmul(feature, w_in[i]), tf.matmul(adj, output_list[i - 1])))
        else:
...
def fc_layer(input_, output_size, activation=None, batch_norm=False, istrain=False, scope=None):
    ...
    with tf.variable_scope(scope or "fc"):
        w = tf.get_variable(name="w", shape=[get_shape(
            input_)[1], output_size], initializer=tf.contrib.layers.xavier_initializer())
        ...
        if batch_norm:
            norm = tf.contrib.layers.batch_norm(
                tf.matmul(input_, w), center=True, scale=True, decay=0.8, is_training=istrain, scope='batch_norm')
